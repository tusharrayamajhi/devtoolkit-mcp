...
const ComplexitySchema = z.object({ file_path: z.string() });

const TodoSchema = z.object({
  dir_path: z.string().nullish().transform(v => v ?? "."),
  glob: z.string().nullish().transform(v => v ?? "**/*.{ts,tsx,js,jsx,py,go,rs,java,cpp,c}"),
  tags: z.array(z.string()).nullish().default(["TODO", "FIXME", "HACK", "NOTE", "XXX"]),
});

const CountSchema = z.object({
  target: z.string(),
  glob: z.string().nullish().transform(v => v ?? "**/*.{ts,tsx,js,jsx,py,go,rs,java}"),
});

const LangSchema = z.object({ file_path: z.string() });

const DupSchema = z.object({
  dir_path: z.string().nullish().transform(v => v ?? "."),
  glob: z.string().nullish().transform(v => v ?? "**/*.{ts,tsx,js,jsx,py}"),
  min_lines: z.number().nullish().transform(v => v ?? 5),
});

const ImportsSchema = z.object({ file_path: z.string() });
...
function countComplexity(content: string): {
  functions: number;
  classes: number;
  ifStatements: number;
  loops: number;
  ...
  for (const line of lines) {
    const l = line.trim();
    if (/\bfunction\b|\b=>\s*{|\bdef\b|\bfn\b/.test(l)) functions++;
    if (/\bclass\b/.test(l)) classes++;
    if (/\bif\b/.test(l)) ifStatements++;
    if (/\bfor\b|\bwhile\b|\bforeach\b/.test(l)) loops++;
...
export async function handleCodeTool(name: string, args: unknown): Promise<string> {
  switch (name) {
    case "analyze_complexity": {
      ...
      try {
        ...
        return safeJson({
          ...
          ...complexity,
          complexity_rating:
            complexity.cyclomaticEstimate <= 10
              ? "Low (good)"
        ...
        throw new ToolError(`Cannot analyze: ${file_path}`, "COMPLEXITY_ERROR", e);
      }
    }
    ...
    case "find_todos": {
      ...
      const results: Array<{ file: string; line: number; tag: string; message: string }> = [];
      ...
      for (const file of files) {
        try {
          ...
          content.split("\n").forEach((line, idx) => {
            ...
            if (match) {
              results.push({
                file: path.relative(dir_path, file),
                line: idx + 1,
      ...
      if (results.length === 0) return "No TODO/FIXME comments found.";
      const grouped = results.reduce<Record<string, typeof results>>((acc, r) => {
        acc[r.tag] = acc[r.tag] ?? [];
        acc[r.tag].push(r);
        return acc;
      }, {});
      return safeJson({ total: results.length, by_tag: grouped });
    }
    ...
    case "count_lines": {
      ...
      try {
        ...
        const files = stat.isDirectory()
          ? await fg(globPattern, { cwd: target, absolute: true, ignore: ["**/node_modules/**", "**/.git/**"] })
          : [target];

        let total = 0, blank = 0, comment = 0, code = 0;
        const fileStats: Array<{ file: string; total: number; code: number }> = [];
        ...
        for (const file of files) {
          try {
            ...
            fileStats.push({ file: path.relative(target, file), total: lines.length, code: fileCode });
          } catch {
            // skip
          }
        }
        ...
        throw new ToolError(`Cannot count lines for: ${target}`, "COUNT_LINES_ERROR", e);
      }
    }
    ...
    case "detect_language": {
      ...
      try {
        const content = await fs.readFile(file_path, "utf-8").catch(() => "");
        ...
        throw new ToolError(`Cannot detect language for: ${file_path}`, "LANG_DETECT_ERROR", e);
      }
    }
    ...
    case "find_duplicates": {
      ...
      for (const file of files) {
        try {
          ...
          for (let i = 0; i <= lines.length - min_lines; i++) {
            ...
            blockMap.get(block)!.push({ file: path.relative(dir_path, file), startLine: i + 1 });
          }
        } catch {
          // skip
        }
      ...
      if (duplicates.length === 0) return "No duplicate blocks found.";
      return safeJson({ duplicate_blocks_found: duplicates.length, duplicates });
    }
    ...
    case "get_imports": {
      ...
      try {
        const content = await fs.readFile(file_path, "utf-8");
        const lines = content.split("\n");
        const imports: Array<{ line: number; statement: string; module: string }> = [];
        ...
        throw new ToolError(`Cannot get imports for: ${file_path}`, "IMPORTS_ERROR", e);
      }
    }

    default:
      throw new ToolError(`Unknown code tool: ${name}`, "UNKNOWN_TOOL");
  }
}
